...
var Ninja = function(options) {
  ...
  this.Block = function(options) {
    ...
    this.Unpair = function(callback) {
      var url = ninja.ServerUrl() + '/block/' + this.Options.nodeId + ninja.Authentication.getAuthSlug();
  ...
  this.Device = function(options) {
    ...
    this.GetCallback = function(callback) {
      var url = ninja.ServerUrl() + '/device/' + this.GUID() + '/callback' + ninja.Authentication.getAuthSlug();
      var xhr = Ninja.Utilities.CreateXHR(function(response) {
        if (response.result) {
          if (callback) {
            callback(response.data);
    ...
    this.SetSubdevice = function(subObject, callback) {
      ...
      if (subObject instanceof ninja.Subdevice) {
        ...
        if (!subObject.Id) {
          url = ninja.ServerUrl() + '/device/' + this.GUID() + '/subdevice' + ninja.Authentication.getAuthSlug();
          xhr = Ninja.Utilities.CreateXHR(function(response) {
            if (response.result) {
              if (callback) {
                callback(response.data);
              }
          ...
          url = ninja.ServerUrl() + '/device/' + this.GUID() + '/subdevice/' + subObject.Id + ninja.Authentication.getAuthSlug();
          xhr = Ninja.Utilities.CreateXHR(function(response) {
            if (response.result) {
              if (callback) {
                callback(true);
    ...
    this.DeleteSubdevice = function(subObject, callback) {
      if (subObject instanceof ninja.Subdevice) {
        ...
        var url = ninja.ServerUrl() + '/device/' + this.GUID() + '/subdevice/' + subObject.Id + ninja.Authentication.getAuthSlug();
        var xhr = Ninja.Utilities.CreateXHR((function(response) {
